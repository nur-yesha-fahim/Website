...
if (isset($_POST['add_service'])) {
    ...
    $status = $_POST['status'];
    $image = $_POST['image']; // Optional image field

    if (empty($title) || empty($price) || empty($delivery_time)) {
        $message = "Please fill in all required fields.";
    } else {
        try {
            $sql = "INSERT INTO services (title, description, bullet_points, price, delivery_time, status, image) 
                    VALUES (?, ?, ?, ?, ?, ?, ?)";
            $stmt = $pdo->prepare($sql);
            if ($stmt->execute([$title, $description, $bullet_points, $price, $delivery_time, $status, $image])) {
...
<body class="text-slate-200 min-h-screen flex items-center justify-center p-6">
    ...
    <div class="glass w-full max-w-2xl p-8 rounded-3xl">
        ...
        <form action="" method="POST" class="space-y-4">
            ...
            <div>
                <label class="block text-[10px] uppercase font-bold text-slate-500 mb-1.5 ml-1">Key Features (Bullet Points)</label>
                <div class="relative">
                    <span class="absolute top-3 left-0 pl-3 text-slate-500">
                        <i class="fa-solid fa-list-ul text-[10px]"></i>
                    </span>
                    <input type="text" name="bullet_points" placeholder="Feature 1, Feature 2, Feature 3"
                           class="w-full bg-slate-950/50 border border-slate-700 rounded-xl p-2.5 pl-10 text-sm outline-none text-white">
                </div>
                <p class="text-[9px] text-slate-500 mt-1 ml-1 italic">* Separate features with commas</p>
            </div>
